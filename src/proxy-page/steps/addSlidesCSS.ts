...
export default (config: ConfigService): Step => (context: ContextService): void => {
  ...
  $('head').append(
    ...
    `<link
      rel="stylesheet"
      href="https://cdnjs.cloudflare.com/ajax/libs/reveal.js/4.2.1/reset.min.css"
    ...
    `<link
      rel="stylesheet"
      href="${basePath}/css/slides.css?cache=${version}"
    />`,
    `<link
      rel="stylesheet"
      href="${basePath}/reveal/theme/${style}.css?cache=${version}"
      id="theme"
    />`,
    // Modern deferred load of stylesheets that are not critical for the first page render
    `<link
      href="https://cdnjs.cloudflare.com/ajax/libs/reveal.js/4.2.1/plugin/highlight/zenburn.min.css"
      rel="preload"
      as="style"
