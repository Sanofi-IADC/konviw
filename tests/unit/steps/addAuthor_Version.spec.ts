...
describe('Confluence Proxy / addTheme', () => {
  ...
  it('should add data-column-id from th to each td (column headers)', () => {
    context.setHtmlBody('<html><head></head><body><div id="Content"><h1 class="titlePage"> Demo table</h1>'+
    '</body></html>');
    step(context);
    expect(context.getHtmlBody()).toContain('<div class="author_header"><img src="Test.jpg" class="author_image"><div class="author_textbox"><p class="author_text">Creator : Test</p><p class="author_text">Page version : 2</p></div></div>');
  });
});
