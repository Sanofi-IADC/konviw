...
describe('ConfluenceProxy / addSlides', () => {
  ...
  it('should add slides', () => {
    ...
    const expectedResult =
      ...
      `<link rel="stylesheet" href="${basePath}/reveal/theme/.css?cache=${version}" id="theme">` +
      `<link href="${basePath}/highlight/zenburn.min.css?cache=${version}" rel="preload" as="style" onload="this.onload=null;this.rel='stylesheet'"></head>` +
      `<body><div id="Content">` +
      ...
      `<span><pre><code><!--?php echo "hello world"; ?--></code></pre></span>` +
      `</section></div>` +
      `</div></div><script defer="" src="${basePath}/reveal/reveal.js?cache=${version}"></script><script defer="" src="${basePath}/reveal/plugin/zoom/zoom.js?cache=${version}"></script>` +
